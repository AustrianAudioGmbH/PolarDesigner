...
//        DBG("MainLookAndFeel Scale Factor: " << Desktop::getInstance().getGlobalScaleFactor());
    ...
    {
        return component ? component->getDesktopScaleFactor() : Desktop::getInstance().getGlobalScaleFactor();
    }
    ...
    void drawButtonBackground (Graphics& g, Button& button, const Colour& backgroundColour, bool isMouseOverButton, bool isButtonDown) override
    {
        ...
        int cornerDirButtons = static_cast<int> (button.getTopLevelComponent()->proportionOfHeight (0.004f) / scale);
        int reduceYDirButtons = static_cast<int> (button.getTopLevelComponent()->proportionOfHeight (0.005f) / scale);
        ...
        else if (button.getButtonText() == "Terminate Spill")
        {
            ...
            Rectangle<float> iconArea = buttonArea.reduced (button.proportionOfWidth (0.45f) * 1.0f, button.proportionOfHeight (0.33f) * 1.0f).translated (button.proportionOfWidth (0.38f) * 1.0f, 0.0f);
        ...
        else if (button.getButtonText() == "Maximize Target")
        {
            ...
            auto iconArea = buttonArea.reduced (button.proportionOfWidth (0.45f) * 1.0f, button.proportionOfHeight (0.33f) * 1.0f).translated (button.proportionOfWidth (0.38f) * 1.0f, 0.0f);
        ...
        else if (button.getButtonText() == "Terminate spill" || button.getButtonText() == "Maximize target" || button.getButtonText() == "Max Target-to-Spill")
        {
        }
        ...
        else if (button.getButtonText() == "01" || button.getButtonText() == "02" || button.getButtonText() == "03")
        {
            if (button.isEnabled())
            {
                Path path;
        ...
        else if (button.getButtonText() == "Click on the button below to apply polar\npatterns with minimum spill energy" || button.getButtonText() == "Click on the button below to apply polar\npatterns with maximum signal energy" || button.getButtonText() == "Find best compromise between reduction\nof spill and maximizing target signal")
        {
        }
        else if (button.getButtonText() == "Begin Terminate" || button.getButtonText() == "Begin Maximize" || button.getButtonText() == "Apply Max Target-to-Spill")
        {
            g.setColour (mainTextColor);
            g.drawRect (buttonArea, 1);
    ...
    void drawButtonText(Graphics& g, TextButton& button, bool /*isMouseOverButton*/, bool /*isButtonDown*/) override
    {
        ...
        else if (button.getButtonText() == "Eight Pattern" || button.getButtonText() == "HyperCardioid Pattern" || button.getButtonText() == "SuperCardioid Pattern" || button.getButtonText() == "Cardioid Pattern" || button.getButtonText() == "BCardioid Pattern" || button.getButtonText() == "Omni Pattern" || button.getButtonText() == "RevBCardioid Pattern" || button.getButtonText() == "RevCardioid Pattern")
        {
            return;
        }
        else if (button.getButtonText() == "Trim Slider Pointer")
    ...
    Slider::SliderLayout getSliderLayout(Slider& slider) override
    {
        ...
        if (slider.getSliderStyle() == Slider::SliderStyle::LinearHorizontal)
        {
            ...
            layout.textBoxBounds.setBounds(static_cast<int>(localBounds.getWidth() * 0.71f), localBounds.getY(), static_cast<int>(localBounds.getWidth() * 0.29f / scale), static_cast<int>(localBounds.getHeight() / scale));
            layout.textBoxBounds.reduce(static_cast<int>(10.0f / scale), static_cast<int>(10.0f / scale));
        }
        else if (slider.getSliderStyle() == Slider::SliderStyle::LinearVertical)
        {
            ...
            int textBoxWidth = static_cast<int>(slider.getTopLevelComponent()->getWidth() * 0.05f / scale);
            int textBoxHeight = static_cast<int>(slider.getTopLevelComponent()->getHeight() * 0.029f / scale);
    ...
    void drawLinearSliderBackground(Graphics& g, int x, int y, int width, int height, float sliderPos, float minSliderPos, float maxSliderPos, const Slider::SliderStyle style, Slider& slider) override
    {
        ...
        {
            Rectangle<float> backgroundRect(x * 1.0f, 0.5f * height - h / 2, width * 1.0f, h * 1.0f);
            pathBgr.addRoundedRectangle(backgroundRect, h);
            Rectangle<float> foregroundRect((width / 2 + x) * 1.0f, 0.5f * height - h / 2, (sliderPos - (width / 2 + x) * 1.0f), h * 1.0f);
    ...
    void drawLabel(Graphics& g, Label& label) override
    {
        ...
        int h = static_cast<int>(static_cast<float>(label.getTopLevelComponent()->getHeight()) * 0.018f / scale);
    ...
    void drawToggleButton(Graphics& g, ToggleButton& button, bool isMouseOverButton, bool isButtonDown) override
    {
        ...
        else
        {
            g.setColour(button.isEnabled() ? textButtonActiveRedFrameColor : textButtonActiveRedFrameColor.withAlpha(0.5f));
            Path outline;
            outline.addRoundedRectangle(toggleButtonBounds.reduced((button.getWidth() * 0.19f), (button.getHeight() * 0.28f)), button.getHeight() * 0.23f / scale, button.getHeight() * 0.23f / scale);
            ...
            drawTickBox(g, button, toggleButtonBounds.getX(), toggleButtonBounds.getY(), toggleButtonBounds.getWidth(), toggleButtonBounds.getHeight(), button.getToggleState(), false, false, false);
        }
    }
    ...
    void drawScrollbar(Graphics& g, ScrollBar& scrollbar, int x, int y, int width, int height, bool isScrollbarVertical, int thumbStartPosition, int thumbSize, bool isMouseOver, bool isMouseDown) override
    {
        ...
        Path pathBgr;
        pathBgr.addRoundedRectangle(x * 1.0f, y * 1.0f, width * 1.0f, height * 1.0f, (width / 2.0f) * 1.0f / scale);
        g.setColour(textButtonFrameColor);
        g.fillPath(pathBgr);
        Path pathFgr;
        pathFgr.addRoundedRectangle(0.0f, thumbStartPosition * 1.0f, width * 1.0f, thumbSize * 1.0f, (width / 2.0f) * 1.0f / scale);
        g.setColour(sliderHoverFrameColor);
        g.fillPath(pathFgr);
    }
...
private:
    ...
    {
        float scale = getScaleFactor(nullptr); // Use global scale factor since no specific component is passed
        g.setColour(labelBackgroundColor);
        ...
        g.fillRoundedRectangle(buttonArea.reduced(deltaX * 1.0f, deltaY * 1.0f), corner * 1.0f / scale);
