    ...
    termStage (PolarDesignerAudioProcessorEditor::terminatorStage::DISABLED)
{
    ...
    tbZeroLatencyAtt = std::unique_ptr<ButtonAttachment> (
        new ButtonAttachment (valueTreeState, "zeroLatencyMode", tbZeroLatency));
    ...
    for (unsigned int i = 0; i < MAX_EDITOR_BANDS; ++i)
    {
        ...
        if (i == (nActiveBands - 1))
        {
            tmbNrBandsButton[static_cast<int> (i)].setToggleState (
                true,
    ...
    tgbProxCtrAtt = std::unique_ptr<ButtonAttachment> (
        new ButtonAttachment (valueTreeState, "proximityOnOff", tgbProxCtr));
    tgbProxCtr.addListener (this);

    addAndMakeVisible (&slProximity);
    slProximityAtt = std::unique_ptr<SliderAttachment> (
        new SliderAttachment (valueTreeState, "proximity", slProximity));
    ...
    polarDesignerProcessor.termControlWaveform.setColours (mainLaF.labelBackgroundColor,
                                                           mainLaF.textButtonActiveRedFrameColor);
    ...
    for (unsigned int i = 0; i < MAX_EDITOR_BANDS; ++i)
    {
        ...
        tgbSoloAtt[i] = std::unique_ptr<ButtonAttachment> (
            new ButtonAttachment (valueTreeState, "solo" + String (i + 1), tgbSolo[i]));
        ...
        tgbMuteAtt[i] = std::unique_ptr<ButtonAttachment> (
            new ButtonAttachment (valueTreeState, "mute" + String (i + 1), tgbMute[i]));
        ...
        slDirAtt[i] = std::unique_ptr<SliderAttachment> (
            new SliderAttachment (valueTreeState, "alpha" + String (i + 1), slDir[i]));
        ...
        slBandGainAtt[i] = std::unique_ptr<SliderAttachment> (
            new SliderAttachment (valueTreeState, "gain" + String (i + 1), slBandGain[i]));
        ...
        directivityEqualiser.addSliders (eqColours[i],
                                         &slDir[i],
                                         (i > 0) ? &slCrossoverPosition[i - 1] : nullptr,
                                         (i < (MAX_EDITOR_BANDS - 1)) ? &slCrossoverPosition[i]
                                                                      : nullptr,
                                         &tgbSolo[i],
                                         &tgbMute[i],
                                         &slBandGain[i],
                                         &polarPatternVisualizers[i]);
    ...
    tbAllowBackwardsPatternAtt = std::unique_ptr<ButtonAttachment> (
        new ButtonAttachment (valueTreeState, "allowBackwardsPattern", tbAllowBackwardsPattern));
...
PolarDesignerAudioProcessorEditor::~PolarDesignerAudioProcessorEditor()
{
    ...
    for (int i = 0; i < MAX_EDITOR_BANDS; ++i)
    {
        ...
        if (i < MAX_EDITOR_BANDS - 1)
            slCrossoverPosition[i].removeListener (this);
    }
    ...
    for (int i = 0; i < 4; ++i)
        tmbSyncChannelButton[i].removeListener (this);
...
void PolarDesignerAudioProcessorEditor::resized()
{
    ...
    polarVisualizersComponent.items.add (
        juce::FlexItem().withWidth (polarVisualizersComponentLeftMargin));
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        muteSoloComponent[i].items.add (juce::FlexItem (tgbSolo[i])
                                            .withMinWidth (muteSoloButtonSize)
                                            .withHeight (muteSoloButtonSize));
        muteSoloComponent[i].items.add (juce::FlexItem().withWidth (muteSoloButtonSpace));
        muteSoloComponent[i].items.add (juce::FlexItem (tgbMute[i])
                                            .withMinWidth (muteSoloButtonSize)
                                            .withHeight (muteSoloButtonSize));
        muteSoloComponent[i].items.add (juce::FlexItem().withFlex (0.4f));
    }
    ...
    gainBandSlidersComponentWrapper.items.add (
        juce::FlexItem().withWidth (polarVisualizersComponentLeftMargin));
    ...
    if (nActiveBands < 2)
    {
        if (polarPatternVisualizers[0].isPvisActive())
        {
            ...
            float bandFlexRelToPolarVisComp =
                bandLimitWidth[0] / (dirEqSize + polarRightMarginDiff);
            float polarVisFlex = bandFlexRelToPolarVisComp / 2 + polarVisHalfWidthFlex;
            polarVisualizersComponent.items.add (
                juce::FlexItem (polarPatternVisualizers[0]).withFlex (polarVisFlex));
            ...
            gainBandSlidersComponent.items.add (
                juce::FlexItem (slBandGain[0]).withFlex (gainSliderFlex));
    ...
    else
    {
        for (unsigned int i = 0; i < nActiveBands; i++)
        {
            if (polarPatternVisualizers[i].isPvisActive())
            {
                ...
                muteSoloModule.items.add (
                    juce::FlexItem (muteSoloComponent[i]).withFlex (bandFlex));

                //Calculate polar visualizers position and size
                auto polarVisHalfWidth = (dirEqSize * 0.135f) / 2;
                if (nActiveBands >= 4)
                    polarVisHalfWidth = (dirEqSize * 0.115f) / 2;
                float polarVisHalfWidthFlex =
                    polarVisHalfWidth / (dirEqSize + polarRightMarginDiff);
                float bandFlexRelToPolarVisComp =
                    bandLimitWidth[i + 1] / (dirEqSize + polarRightMarginDiff);
                float polarVisFlex = bandFlexRelToPolarVisComp / 2 + prevPolarVisFlex / 2;
                //Add polarVisHalfWidthFlex only on first iteration
                if (i == 0)
                    polarVisFlex = bandFlexRelToPolarVisComp / 2 + prevPolarVisFlex / 2
                                   + polarVisHalfWidthFlex;
                polarVisualizersComponent.items.add (
                    juce::FlexItem (polarPatternVisualizers[i]).withFlex (polarVisFlex));
                ...
                if (i == 0)
                    gainSliderFlex = bandFlex / 2 + prevGainSliderFlex / 2 + pixe1lLine / dirEqSize
                                     + gainSliderHalfWidthFlex;
                gainBandSlidersComponent.items.add (
                    juce::FlexItem (slBandGain[i]).withFlex (gainSliderFlex));
                ...
                if (i == nActiveBands - 1)
                    gainBandSlidersComponent.items.add (
                        juce::FlexItem().withFlex (1 - gainSliderFlexSum));
            }
        }
    }
    int gainRightMargin =
    ...
    tbTrimSliderCenterPointerComponent.items.add (
        juce::FlexItem (tbTrimSliderCenterPointer).withFlex (0.85f));
    ...
    directivityEqualiserComponent.items.add (
        juce::FlexItem (directivityEqualiser).withFlex (0.94f));
    directivityEqualiserComponent.items.add (
        juce::FlexItem (tbTrimSliderCenterPointerComponent).withFlex (0.02f));
    directivityEqualiserComponent.items.add (
        juce::FlexItem (tbTrimSliderComponent).withFlex (0.04f));

    gainBandSlidersComponentWrapper.items.add (
        juce::FlexItem (gainBandSlidersComponent).withFlex (1.f));
    ...
    fbProximityControlInComp.items.add (
        juce::FlexItem { fbProximityControlToggleButton }.withFlex (0.5f));
    ...
    fbTerminatorControlCloseComp.items.add (
        juce::FlexItem { tbCloseTerminatorControl }.withFlex (0.12f));
    ...
    fbTermLbMaxToSpill.items.add (
        juce::FlexItem { terminatorLabelMaxToSpillMain }.withFlex (0.88f));
    ...
    fbTermLbMaxToSpillDesc.items.add (
        juce::FlexItem { terminatorLabelMaxToSpillSub }.withFlex (0.88f));
    ...
    if (uiTerminatorAnimationWindowIsVisible)
    {
        ...
        fbTerminatorControlInComp.items.add (
            juce::FlexItem { fbTerminatorControlCloseComp }.withFlex (0.12f));
        fbTerminatorControlInComp.items.add (juce::FlexItem {}.withFlex (0.06f));
        fbTerminatorControlInComp.items.add (juce::FlexItem {
            uiTargetAquisitionWindowIsVisible ? albAcquiringTarget : albPlaybackSpill }
                                                 .withFlex (0.22f));
        fbTerminatorControlInComp.items.add (
            juce::FlexItem { polarDesignerProcessor.termControlWaveform }.withFlex (0.46f));
        fbTerminatorControlInComp.items.add (juce::FlexItem {}.withFlex (0.06f));
    }
    else
    {
        if (uiMaxToSpillWindowIsVisible)
        {
            ...
            switch (termStage)
            {
                ...
                case terminatorStage::TERMINATE:
                    ...
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbSpill }.withFlex (0.1f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbEmptyLine1 }.withFlex (0.01f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbSpillDesc }.withFlex (0.14f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbEmptyLine2 }.withFlex (0.01f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbSpillBegin }.withFlex (0.16f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbEmptyLine3 }.withFlex (0.06f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbMax }.withFlex (0.1f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbEmptyLine4 }.withFlex (0.02f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbMaxToSpill }.withFlex (0.1f));
                    fbTerminatorControlInComp.items.add (juce::FlexItem {}.withFlex (0.06f));
                    break;
                case terminatorStage::MAXIMIZE:
                    ...
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbSpill }.withFlex (0.1f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbEmptyLine1 }.withFlex (0.02f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbMax }.withFlex (0.1f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbEmptyLine2 }.withFlex (0.01f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbMaxDesc }.withFlex (0.14f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbEmptyLine3 }.withFlex (0.01f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbMaxBegin }.withFlex (0.16f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbEmptyLine4 }.withFlex (0.06f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbMaxToSpill }.withFlex (0.1f));
                    fbTerminatorControlInComp.items.add (juce::FlexItem {}.withFlex (0.06f));
                    break;
                case terminatorStage::MAXTOSPILL:
                    ...
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbMax }.withFlex (0.1f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbEmptyLine2 }.withFlex (0.02f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbMaxToSpill }.withFlex (0.1f));
                    fbTerminatorControlInComp.items.add (juce::FlexItem {}.withFlex (0.01f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbMaxToSpillDesc }.withFlex (0.14f));
                    fbTerminatorControlInComp.items.add (juce::FlexItem {}.withFlex (0.01f));
                    fbTerminatorControlInComp.items.add (
                        juce::FlexItem { fbTermLbMaxToSpillApply }.withFlex (0.16f));
                    fbTerminatorControlInComp.items.add (juce::FlexItem {}.withFlex (0.06f));
                    break;
                default:
                    break;
        ...
        else
        {
            ...
            fbTerminatorControlInComp.items.add (
                juce::FlexItem { tbTerminateSpill }.withFlex (0.22f));
            fbTerminatorControlInComp.items.add (juce::FlexItem {}.withFlex (0.01f));
            fbTerminatorControlInComp.items.add (
                juce::FlexItem { tbMaximizeTarget }.withFlex (0.22f));
            fbTerminatorControlInComp.items.add (juce::FlexItem {}.withFlex (0.01f));
            fbTerminatorControlInComp.items.add (
                juce::FlexItem { tbMaxTargetToSpill }.withFlex (0.22f));
            fbTerminatorControlInComp.items.add (juce::FlexItem {}.withFlex (0.06f));
        }
    }
...
void PolarDesignerAudioProcessorEditor::buttonClicked (Button* button)
{
    ...
    { // disable tmbABButton group if syncChannels are enabled, as AB is inapplicable in that case
        ...
        valueTreeState.getParameter ("syncChannel")
            ->setValueNotifyingHost (
                valueTreeState.getParameter ("syncChannel")->convertTo0to1 ((1)));
    }
    if ((button == &tmbSyncChannelButton[1]) && (button->getToggleState()))
    {
        ...
        valueTreeState.getParameter ("syncChannel")
            ->setValueNotifyingHost (
                valueTreeState.getParameter ("syncChannel")->convertTo0to1 ((2)));
    }
    if ((button == &tmbSyncChannelButton[2]) && (button->getToggleState()))
    {
        ...
        valueTreeState.getParameter ("syncChannel")
            ->setValueNotifyingHost (
                valueTreeState.getParameter ("syncChannel")->convertTo0to1 ((3)));
    }
    if ((button == &tmbSyncChannelButton[3]) && (button->getToggleState()))
    {
        ...
        valueTreeState.getParameter ("syncChannel")
            ->setValueNotifyingHost (
                valueTreeState.getParameter ("syncChannel")->convertTo0to1 ((4)));
    }

    // Re-enable the tmbABButtons if tmbSyncChannelButton is de-selected
    if (((button == &tmbSyncChannelButton[0]) || (button == &tmbSyncChannelButton[1])
                                                                               ...
                                                                               float offsetPolVis)
{
    ...
    {
        bandLimit.push_back (
            static_cast<float> (directivityEqualiser.getBandWidth (static_cast<int> (i))));
    }
    ...
    {
        bandLimitWidth.push_back (bandLimit[static_cast<unsigned long> (i)]
                                  - bandLimit[static_cast<unsigned long> (i - 1)]);
    }
    ...
    if (uiTerminatorAnimationWindowIsVisible)
    {
        if (polarDesignerProcessor.playHeadPosition.getIsPlaying())
        {
            if (! uiTargetAquisitionWindowIsVisible)
            {
                ...
                activateMainUI (false);
                setMainAreaEnabled (false);
                polarDesignerProcessor.startTracking (uiMaximizeTargetWindowIsVisible ? false
                                                                                      : true);
                albAcquiringTarget.startAnimation ("ACQUIRING TARGET   ",
                                                   uiMaximizeTargetWindowIsVisible
                                                       ? "STOP PLAYBACK WHEN READY TO MAXIMIZE  "
...
int PolarDesignerAudioProcessorEditor::getControlParameterIndex (Component& control)
{
    if (&control == &directivityEqualiser.getBandlimitPathComponent (0) && nActiveBands > 1)
        ...
        return 4;
    else if ((&control == &slDir[1] || &control == &directivityEqualiser.getDirPathComponent (1))
             && nActiveBands > 1)
        return 5;
    else if ((&control == &slDir[2] || &control == &directivityEqualiser.getDirPathComponent (2))
             && nActiveBands > 2)
        return 6;
    else if ((&control == &slDir[3] || &control == &directivityEqualiser.getDirPathComponent (3))
             && nActiveBands > 3)
        return 7;
    else if ((&control == &slDir[4] || &control == &directivityEqualiser.getDirPathComponent (4))
             && nActiveBands > 4)
        return 8;
